...
// Simple fallback for when Vue is not available
if (typeof Vue === 'undefined') {
    // Create a minimal app implementation
...
Generated by AI Marketing Assistant
        ...
        document.addEventListener('click', (e) => {
            if (e.target.closest('button[onclick="app.generateNewStrategy()"]') || 
                e.target.textContent.includes('Generate New Strategy')) {
                app.generateNewStrategy();
            } else if (e.target.closest('button[onclick="app.copyResults()"]') || 
                       e.target.textContent.includes('Copy Results')) {
                app.copyResults();
            } else if (e.target.closest('button[onclick="app.clearError()"]') || 
    ...
    createApp({
        ...
        methods: {
            // Vue methods go here (existing implementation)
        }
    }).mount('#app');
}